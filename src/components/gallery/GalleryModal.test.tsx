...
describe('GalleryModal', () => {
  ...
  it('renders correctly when open and applies modernized description styles', () => {
    ...
    if (imageCenteringContainer) {
      // Verify it no longer has grid centering classes
      expect(imageCenteringContainer.className).not.toContain('grid');
      expect(imageCenteringContainer.className).not.toContain('place-items-center');
      // Ensure it does not have flex centering classes either
      expect(imageCenteringContainer.className).not.toContain('flex');
      expect(imageCenteringContainer.className).not.toContain('items-center');
      expect(imageCenteringContainer.className).not.toContain('justify-center');
      // Check for essential layout and positioning classes
      ...
      // Check the img tag itself for new styling
      const imgElement = imageCenteringContainer.querySelector('img');
      ...
      if (imgElement) {
        ...
        expect(imgElement.className).not.toContain('max-w-full');
        expect(imgElement.className).not.toContain('max-h-full');
        expect(imgElement.className).not.toContain('object-contain');
        // Verify alt text is still present
        expect(imgElement.alt).toBe(mockImages[0].altKey);
      }
    }
